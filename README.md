...
$J$ averages the return over trajectories generated by $\pi_\theta$, so evaluating it at new $\theta$ requires new episodes. PPO instead reuses one batch from $\pi_{\theta_{old}}$ for several gradient steps, reweighting each step by the importance sampling ratio $\rho_t(\theta)$. The clip keeps $\pi_\theta$ close to $\pi_{\theta_{old}}$, where this surrogate is accurate.

PPO algorithm trains the actor model by gradient ascent on the clipped objective function.

$$
L^{CLIP}(\theta) = \mathbb{E}_t\Big[\min\big(\rho_t(\theta)\,A_t,\ \text{clip}\big(\rho_t(\theta),\,1-\epsilon,\,1+\epsilon\big)\,A_t\big)\Big]
$$
